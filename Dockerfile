...
# -- it would fail against this read-mostly, non-root filesystem.
RUN useradd -m -u 1000 appuser && chown -R appuser:appuser /app /opt/hf-cache
USER appuser
...
# itself lives under data/demo/.
COPY data/demo/ ./data/demo/
COPY data/processed/term_idf.json ./data/processed/term_idf.json
...
CMD gunicorn -k uvicorn.workers.UvicornWorker -w ${WEB_CONCURRENCY:-1} -b 0.0.0.0:${APP_PORT} --timeout 120 src.talentrank.api:app
